...
public class DecalGroupStrategy implements GroupStrategy, Disposable {
	...
	public float[] camCoords = new float[3];
	Camera camera;
	ShaderProgram shader;
	private final Comparator<Decal> cameraSorter;
	public IUniformSetter uniformSetter;
	
	public DecalGroupStrategy(final Camera camera, ShaderProgram shader, IUniformSetter uniformSetter) {
		this(camera, shader);
		this.uniformSetter = uniformSetter;
		
	}

	public DecalGroupStrategy (final Camera camera, ShaderProgram shader) {
	...
	public DecalGroupStrategy (Camera camera, Comparator<Decal> sorter, ShaderProgram shader) {
		this.camera = camera;
	...
	public void beforeGroups () {
		...
		camCoords[0] = camera.position.x;
		camCoords[1] = camera.position.y;
		camCoords[2] = camera.position.z;
		shader.setUniform3fv("camCoords", camCoords, 0, camCoords.length);
